...
- [下载正式签名 APK](https://github.com/N3urda/hongguoTV/releases/tag/kotlin-v0.3.0)
...
**红果账号同步尚未实现**：你使用抖音账号登录红果，仍需要可验证的红果授权、账号映射和收藏/进度读写接口。当前收藏与观看记录继续保存在本机。调查依据和接入条件见 [账号同步调查](../docs/ACCOUNT_SYNC.md)。两个二维码均不是红果登录入口。
...
1. 将 `hongguotv-kotlin-0.3.0-android8.apk` 拷贝到电视，允许文件管理器安装应用后打开 APK；或执行 `adb install -r hongguotv-kotlin-0.3.0-android8.apk`。
...
# 输出：outputs/hongguotv-kotlin-0.3.0-android8.apk
```
...
播放器直接通过 HTTPS 分段拉取数据，经本机 Kotlin 处理后交给 Media3。没有 JS 引擎或常驻内容服务。仅在用户打开「手机输入」窗口时，APK 临时监听局域网 HTTP 输入；接收一个搜索词、超时或关闭窗口后停止，不处理账号凭据或媒体。媒体只在内存中处理，不下载整部剧到存储。元数据响应、封面大小、索引数量和播放缓冲均有上限。
